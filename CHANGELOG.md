...
- **`npm run psi` was silently reusing cached PSI reports instead of running fresh audits.** The PSI API caches results per exact URL; repeated checks against the same production URL returned the same cached report every time, which made a batch of "5 runs" look artificially stable (identical scores) and hid the real mobile Performance variance the repo owner was seeing in manual PSI web-UI checks. `scripts/psi.ts` now appends a `psi_cache_bust=<timestamp>` query param to force a real, fresh audit on every call, and prints LCP/TBT/SI/FCP/CLS alongside the category scores so a regression is visible immediately instead of only in the category score.
- **Mobile Performance regression from below-the-fold code-splitting, reverted.** PR #130 converted `ProjectsGrid`/`Timeline`/`GitHubActivity`/`Contact`/`SkillsSlider` to `next/dynamic()` to cut main-thread script-evaluation time, but once the cache-busting fix above exposed real measurements, mobile Performance was landing reliably in the low-to-mid 80s (previously misread as a stable 93 due to the caching bug). Lighthouse's simulated mobile network throttling appears to penalize the extra chunk/request count more than the script-eval savings were worth. Reverted `IntroGate.tsx` to plain static imports for all five components; 5 fresh cache-busted production runs afterward landed at 87, 94, 94, 96, 96 (mobile Performance), versus 81-87 before the revert.
