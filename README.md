...
- **Skills Slider** — Dual auto-scrolling carousels (opposite directions) with 34 technology icons, plus a Recharts radar chart for expertise-area breakdown.
...
- **SEO** — JSON-LD structured data (`Person` + `WebSite` schema), Open Graph image generated via `next/og`, canonical URL, `sitemap.xml` and `robots.txt` auto-generated by App Router, and a static `llms.txt` describing the site for AI agents/crawlers.
...
└── lib/
    ...
    ├── translations/           # All UI strings for EN and PT-BR — one file per section (nav, hero, skills, …) so a component only pulls in its own strings, not the whole app's
...
├── llms.txt                    # AI-agent/crawler-friendly site summary (llmstxt.org convention)
└── images/
